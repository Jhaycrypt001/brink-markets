...
if (!privateKey) {
  throw new Error("BRINK_TEST_PRIVATE_KEY is missing in this PowerShell session. Set it here, then rerun npm run testnet:wallet.");
}
if (!/^0x[a-fA-F0-9]{64}$/.test(privateKey)) {
  throw new Error("BRINK_TEST_PRIVATE_KEY has the wrong format. It must be 0x followed by exactly 64 hexadecimal characters; do not include < > or spaces.");
}
...
for (const market of Object.values(markets)) {
  if (market.type !== "binary" || !market.active || !market.outcomes?.[0] || !isBinaryMarket(market.info)) continue;
  const binary = market.info;
  const expiry = Number(binary.expiry);
  if (expiry - now < 300) continue;
  ...
  if (amount <= 0 || price <= 0 || price >= 1) continue;
  candidates.push({ market, outcome: market.outcomes[0], expiry, book, onchain, amount, price });
  if (candidates.length >= 5) break;
}

if (candidates.length === 0) throw new Error("No live binary market with expiry headroom and an ask was found.");
...
const collateralFree = Number(balance.tUSDC?.free ?? 0);
const estimatedCost = chosen.amount * chosen.price;

console.log(JSON.stringify({
  phase: "preflight",
  chainId: somniaShannon.id,
  marketId: (chosen.market.info as Extract<typeof chosen.market.info, { marketType: "BINARY" }>).marketId,
...
if (collateralFree < estimatedCost) {
  throw new Error(`Insufficient tUSDC for test order: need approximately ${estimatedCost.toFixed(6)}, available ${collateralFree.toFixed(6)}. Fund the wallet, then rerun preflight.`);
}

const chosenBinary = chosen.market.info as Extract<typeof chosen.market.info, { marketType: "BINARY" }>;
const refreshed = await exchange.client.getMarketOnchain(chosenBinary.marketId);
if (refreshed.status !== 1) throw new Error("Market stopped trading during preflight; refusing to send.");

const finalBook = await exchange.fetchOrderBook(chosen.outcome.symbol, 1);
const finalAsk = finalBook.asks[0]?.[0];
if (finalAsk === undefined) throw new Error("Ask disappeared during preflight; refusing to send.");
...
const receipt = (order.info as { receipt?: { transactionHash?: string; status?: string } }).receipt;
